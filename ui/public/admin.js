...
const Admin = () => {
    ...
    <div class="left">
    <button class="left-button ${ui === UI_USERS ? 'primary' : ''}" onClick=${() => setUi(UI_USERS)}>Users</button>
    <button class="left-button ${ui === UI_USERS ? '' : 'primary'}"  onClick=${() => setUi(UI_MAPS)}>Map files</button>
    </div>
...
const Users = () => {
    ...
    return html`<ul class="list" key="users">
        ...
        (u) =>
            ...
            <button class="form-button" onClick=${() => setEditUser(u)}>Edit</button>
            </div>`
    )}
    ${editUser && html`<${EditUser} onClose=${() => setEditUser(null)} user=${editUser} saveUser=${onSaveUser} error=${error}/>`}
     </ul>`;
};

const EditUser = ({ onClose, user, saveUser, error }) => {
    const [email, setEmail] = useState(user.email)
    const [role, setRole] = useState(user.role)
    return html`<div class="modal">
    <div class="modal-content">
    <button class="close-button" onClick=${onClose}>✕</button>
    <form class="form" onSubmit=${(e) => { e.preventDefault(); saveUser({ id: user.id, email, role }) }}>
    <div class="title">Edit User </div>
    <div>${user.id}</div>
    <div class="label">Email</div>
    <input class="form-input" value=${email} onChange=${(e) => setEmail(e.target.value)} disabled/>
...
const Maps = () => {
    ...
    ${editMap && html`<${EditMap} onClose=${() => setEditMap(null)} map=${editMap} saveMap=${onSaveMap} error=${error}/>`}
     </ul>`;
}

const EditMap = ({ onClose, map, saveMap, error }) => {
    const [name, setName] = useState(map.name)
    const [type, setType] = useState(map.type)
    return html`<div class="modal">
    <div class="modal-content">
    <button class="close-button" onClick=${onClose}>✕</button>
    <form class="form" onSubmit=${(e) => { e.preventDefault(); saveMap({ id: map.id, name, type }) }}>
