...
export const createDBConnection = async (name: string, synchronize: boolean = false) => {
...
const joinDBs = async () => {
  ...
  const files = fs.readdirSync('./data').filter(item => item.includes('mende-')).map(item => item.replace('.sqlitedb', ''))
  console.log(files)
  ...
  try {
    for (const file of files) {
      ...
      for (let i = 0; i <= count + BATCH; i = i + BATCH) {
        ...
        if (tiles.length > 0) {
          const data = tiles.map(({ x, y, z, image }: Tile) => ({ x, y, z, image }))
...
const mergeTiles = async () => {
  ...
  const mainRep = connection.getRepository(Tile)
  try {
    const dubs = await mainRep.createQueryBuilder()
